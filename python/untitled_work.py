...
class Measles(SIR):
    def __init__(self, pars=None, *args, **kwargs):
        ...
        self.default_pars(
            beta=1.9,                            # Placeholder value for transmission rate
            init_prev=ss.bernoulli(p=0.005),     # Initial prevalence
            imm_prob=0.5,                        # Immunity probability
            dur_exp=ss.normal(loc=8/30),        # Incubation period (days)
            dur_inf=ss.normal(loc=11/30),       # Infectious period (days)
    ...
    def set_initial_states(self, sim):
        ...
        initially_immune = imm_prob > np.random.rand() # initially_immune = self.pars.init_imm.filter(alive_uids)
...
measles = Measles()
sim = ss.Sim(pars = pars, diseases = measles, start =2020, dt = 1/12, end=2030)
sim.run()
sim.results
